...
{
	//int uart1_len = ioctl(__uart_dev, UART_RXBUF_LEVEL, UART1);

	struct uart_data u_data1;
	u_data1.uart_ch = UART1;
	u_data1.buf_size = 6;
	u_data1.uart_buf = buf;

	printf("dev:%d, uart1_len:%d\n", __uart_dev, 6);

	ioctl(__uart_dev, UART_DATA_READ, u_data1);
}
